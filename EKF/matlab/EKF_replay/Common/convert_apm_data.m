...
imu_data.time_us = IMT2(:,2);
imu_data.gyro_dt = IMT2(:,5);
imu_data.del_ang = IMT2(:,6:8);
imu_data.accel_dt = IMT2(:,4);
imu_data.del_vel = IMT2(:,9:11);
...
last_time = 0;
output_index = 1;
for source_index = 1:length(MAG2)
    mag_timestamp = MAG2(source_index,2);
    if (mag_timestamp ~= last_time)
        mag_data.time_us(output_index,1) = mag_timestamp;
        mag_data.field_ga(output_index,:) = 0.001*[MAG2(source_index,3),MAG2(source_index,4),MAG2(source_index,5)];
        last_time = mag_timestamp;
        output_index = output_index + 1;
    end
end
...
control.up = IN(:,5);
control.uy = IN(:,6);
folder = '../OutputData/control';
fileName = '../OutputData/control/control.mat';
if ~exist(folder,'dir')
mkdir(folder);
end
...
rcou.u3 = RCOU(:,5)/1000;
rcou.u4 = RCOU(:,6)/1000;
folder = '../OutputData/rcou';
fileName = '../OutputData/control/rcou.mat';
if ~exist(folder,'dir')
mkdir(folder);
end
save(fileName,'rcou');
